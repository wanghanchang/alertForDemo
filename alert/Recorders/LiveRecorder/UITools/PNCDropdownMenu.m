...
@implementation PNCDropdownMenu
...
- (void)createMainBtnWithFrame:(CGRect)frame{
    ...
    if (self.type == Type_alert) {
        [_mainBtn setBackgroundColor:[UIColor colorFromHexString:@"#999999"]];
        ...
        _arrowMark.image  = [UIImage imageNamed:@"dropdownMenu_corner_white"];
        _arrowMark.center = CGPointMake(VIEW_CENTER_X(_arrowMark), VIEW_HEIGHT(_mainBtn)/2);
        [_mainBtn addSubview:_arrowMark];

    } else {
        [_mainBtn setFrame:CGRectMake((SCREENWIDTH / 2) - 60, 0, 120, frame.size.height)];
        [_mainBtn setBackgroundColor:[UIColor whiteColor]];
        [_mainBtn setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
        [_mainBtn setTitle:@"录音文件" forState:UIControlStateNormal];
        [_mainBtn.titleLabel setFont:[UIFont boldSystemFontOfSize:17.0]];
        
        _arrowMark = [[UIImageView alloc] initWithFrame:CGRectMake(_mainBtn.frame.size.width - 25, 0, 8, 8)];
        _arrowMark.image  = [UIImage imageNamed:@"dropdownMenu_corner_pink"];
        _arrowMark.center = CGPointMake(VIEW_CENTER_X(_arrowMark), VIEW_HEIGHT(_mainBtn)/2);
        [_mainBtn addSubview:_arrowMark];
...
- (void)setMenuTitles:(NSArray *)titlesArr titleColors:(NSArray *)colorsArr rowHeight:(CGFloat)rowHeight andNumberArray:(NSMutableArray*)array {
    ...
    _numberArr = array;
    [self setMenuTitles:titlesArr titleColors:colorsArr rowHeight:rowHeight];
}



- (void)setMenuTitles:(NSArray *)titlesArr titleColors:(NSArray *)colorsArr rowHeight:(CGFloat)rowHeight{
    ...
    _colorArr = [NSArray arrayWithArray:colorsArr];
    ...
    if (self.type == Type_record_file) {
        _footView = [[TagEditTwoView alloc] init];
        [_footView setFrame:CGRectMake(0, 0, SCREENWIDTH, _rowHeight)];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.type == Type_alert) {
        ...
        cell.backgroundColor = [UIColor colorFromHexString:_colorArr[indexPath.row]];
        cell.textLabel.text =[_titleArr objectAtIndex:indexPath.row];
        return cell;

    } else {
        DropdownMuneTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:CellIdentifier];
        if (cell == nil) {
            cell = [[DropdownMuneTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:CellIdentifier];
            cell.selectionStyle          = UITableViewCellSelectionStyleNone;
        }
        cell.colorNameLabel.text =[_titleArr objectAtIndex:indexPath.row];
        cell.colorNameLabel.textColor = [UIColor colorFromHexString:_colorArr[indexPath.row]];
        cell.colorView.backgroundColor = [UIColor colorFromHexString:_colorArr[indexPath.row]];
        cell.colorCount.textColor = [UIColor colorFromHexString:_colorArr[indexPath.row]];
        if (_numberArr.count > 0) {
            cell.colorCount.text = _numberArr[indexPath.row];
        }
        return cell;
    }
    return nil;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.type == Type_alert) {
        ...
        [_mainBtn setBackgroundImage:[UIColor imageFromHexString:_colorArr[indexPath.row]] forState:UIControlStateNormal];
    } else {
        DropdownMuneTableViewCell *cell = [tableView cellForRowAtIndexPath:indexPath];
        [_mainBtn setTitle:cell.colorNameLabel.text forState:UIControlStateNormal];
        [_mainBtn setTitleColor:[UIColor colorFromHexString:_colorArr[indexPath.row]] forState:UIControlStateNormal];
        [_mainBtn setBackgroundColor:[UIColor whiteColor]];
    }
